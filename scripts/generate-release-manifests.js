  ...
  if (releaseRes.status === 404) {
    ...
    const createRes = await fetch(createUrl, {
      ...
      body: JSON.stringify({
        ...
        body: `### QC Manager v${version} (Multi-Device & Copy Helper Release)\n\nThis release focuses on multi-device access, Copy Helper for everyone, and reporting accuracy:\n\n- **Multi-Device Login:** A user can now stay logged in simultaneously on Web, Desktop, and Android (up to 10 devices/browsers). Logging in on a new device or logging out of one device no longer signs out the others.\n- **Copy Helper for All Users:** The Copy Helper dashboard is now available to every authenticated user (previously superadmin-only). Section visibility is driven by the Sale file-type permission, with dynamic box numbering — users without Sale permission see the Detailed Report only.\n- **Admin Sales Summary:** New "Sales Report for Admin" box with intelligent deduplication — multiple submissions of the same file count once, Sold status takes priority over Unsold, and repeat sales of the same file name count as separate sales. Computed server-side for accuracy across all users.\n- **Navbar Rank Accuracy:** The rank shown next to the user's name now always matches the Leaderboard page — the brief incorrect "#1" flash for badged users has been eliminated (single RPC-driven source of truth).\n- **Performance:** The Word-document converter now loads on demand (smaller initial bundle on all platforms), and the oversized Apple touch icon was reduced by 98%.`,
        draft: false,
        prerelease: false
      })
    });
