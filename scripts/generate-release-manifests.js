  ...
  if (releaseRes.status === 404) {
    ...
    const createRes = await fetch(createUrl, {
      ...
      body: JSON.stringify({
        ...
        body: `### QC Manager v${version} (Patch Release)\n\nWelcome to the patch release of **QC Manager**!\n\nThis release includes the following fixes and enhancements:\n\n- **Resilient Overwrite & Retry Pipeline:** Implemented space-insensitive lookup and space-to-dot filename normalization to prevent duplicate/already_exists upload failures, with 3-retry upload handlers on network race conditions.\n- **Secure Android Signing & Verification:** Configured automated zipalign, apksigner v1/v2/v3/v4 verification, metadata check using aapt badging, and minimum file size threshold checks to ensure only valid signed APKs are published.\n- **Tauri & Android App Icons Refresh:** Integrated optimized high-res launcher assets across both Tauri desktop (icns/ico) and Capacitor Android (mipmap layers).\n- **Log Spam & Egress Fixes:** Resolved overnight localhost log warning spam and Supabase mobile updater network polling egress leaks in development.`,
        draft: false,
        prerelease: false
      })
    });
