...
export const UserStats: React.FC<UserStatsProps> = ({
  ...
  const [selectedPref, setSelectedPref] = useState<"reserve" | "paid">(
    "reserve",
  );

  const handleEditHolidayResponse = async (
    holidayDate: string,
    holidayName: string,
  ...
  const hasH2Carryover = halfYearlyStats
    ? halfYearlyStats.carryForward > 0
    : false;

  const formatDaysAndHoursNode = (
    daysVal: number,
    ...
    const hrPart =
      hours > 0
        ? `${String(hours).padStart(2, "0")} hr${hours > 1 ? "s" : ""}`
        : "";
    const minPart =
      mins > 0
        ? `${String(mins).padStart(2, "0")} min${mins > 1 ? "s" : ""}`
        : "";
            ...
            <div className="bg-theme-card-bg border border-theme-border-input shadow-2xl rounded-2xl w-full max-w-md p-6 relative overflow-hidden font-sans">
                  ...
                  <div className="flex justify-between items-start">
                    <div>
                      ...
                      <span className="text-[11px] font-mono text-teal-400 font-bold">
                        {formatDate(editPrefHoliday.date)}
                      </span>
                    </div>
                    <span
                ...
                <div className="space-y-3">
                  ...
                  <div className="grid grid-cols-1 gap-2.5">
                    ...
                    >
                      ...
                      >
                        {selectedPref === "reserve" && (
                          <div className="w-2 h-2 rounded-full bg-teal-400 animate-scale-up" />
                        )}
                      </div>
                      <div>
                    ...
                    >
                      ...
                      >
                        {selectedPref === "paid" && (
                          <div className="w-2 h-2 rounded-full bg-emerald-400 animate-scale-up" />
                        )}
                      </div>
                      <div>
                        <span className="text-xs font-bold text-theme-text-primary block">
                    ...
                    <div className="bg-purple-955/20 border border-purple-900/50 p-3.5 rounded-xl flex items-start gap-2.5 animate-in slide-in-from-top-1 duration-200">
                      ...
                      <div className="text-[10.5px] text-purple-300 leading-relaxed font-medium">
                        <span className="font-bold text-purple-400 block mb-0.5">
                          ⚠️ Warning: Unadjusting Leaves
                        </span>
                        Changing to 'Get Paid' will set any leaves previously
                        adjusted against this holiday to 'No Adjustment'. These
                        leaves will now count as standard Full Leave days
                        instead.
                      </div>
                    </div>
                  )}
              ...
              <div className="mt-6 pt-4 border-t border-theme-border-input/80 flex justify-end gap-2.5">
                ...
                <button
                  ...
                  className="px-4 py-2 bg-linear-to-r from-teal-600 to-emerald-600 hover:from-teal-550 hover:to-emerald-550 text-white rounded-lg text-xs font-bold shadow-md shadow-teal-950/20 hover:shadow-teal-900/30 cursor-pointer transition-all flex items-center gap-1.5 disabled:opacity-50"
                >
