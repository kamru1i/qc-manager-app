...
interface EUICausalityPanelProps {
...
const getDriverDefaultTitles = (id: number): TitleItem[] => {
  ...
  return [
    ...
    { id: `drv-${id}-occupation`, text: "   * Occupation –" },
    { id: `drv-${id}-industry`, text: "        * Industry –" }
  ];
};

export const EUICausalityPanel: React.FC<EUICausalityPanelProps> = ({
  profile,
  isOnline
}) => {
  ...
  // Helper to dynamically calculate current display list of main applicant titles with sequential numbering
  const getFormattedMainTitles = (): string[] => {
    const lines: string[] = [];
    let counter = 1;
    ...
    mainTitles.forEach((t) => {
      ...
      if (t.id === "eui-occupation" || t.id === "eui-industry") {
        lines.push(t.text);
        return;
      }
      if (t.id === "eui-parking-day" || t.id === "eui-parking-night") {
        lines.push(t.text);
        return;
      }

      // Inject relationship fields dynamically right before NCD
      if (t.id === "eui-ncd" && drivers.length > 0) {
        drivers.forEach((d) => {
          const paddedId = String(d.id).padStart(2, "0");
          lines.push(`${counter}. Relationship with the AD ${paddedId} -`);
          counter++;
        });
      }

      const cleanText = t.text.replace(/^\d+\.\s*/, "");
      if (t.id === "eui-blank-10") {
        lines.push(`${counter}.`);
      } else {
  ...
  // Helper to dynamically calculate field serial numbers for edit form labels
  const getNumberedMap = () => {
    ...
    mainTitles.forEach((t) => {
      if (
        ...
        t.id === "eui-occupation" ||
        t.id === "eui-industry" ||
        t.id === "eui-parking-day" ||
        t.id === "eui-parking-night"
      ) {
        return;
      }
      if (t.id === "eui-ncd" && drivers.length > 0) {
  ...
  const numberedMap = getNumberedMap();
  const getNumberDisplay = (id: string) => {
    const num = numberedMap[id];
    ...
    try {
      ...
      if (data && data.code) {
        ...
        if (parsed.mainTitles) {
          // Remove legacy hardcoded numbers and relations from DB to sync cleanly with dynamic numbering logic
        ...
        if (savedDraft) {
          ...
          if (parsed.mainTitles) {
            const cleanTitles = parsed.mainTitles.map((t: any) => ({
              ...t,
              text: t.text.replace(/^\d+\.\s*/, "")
            }));
            setMainTitles(cleanTitles);
  ...
  useEffect(() => {
    if (!dbLoading) {
      localStorage.setItem(
        "quotes_eui_causality_template_draft",
  ...
  const getCopyFormattedText = (): string => {
    let result = "";
    const formattedMain = getFormattedMainTitles();
    formattedMain.forEach((line) => {
      result += `${line}\n`;
    });
    drivers.forEach((d) => {
      result += "\n";
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-4 border-b border-slate-850/60 pb-4 border-dashed">
        <div>
          <h4 className="text-md font-bold text-white flex items-center gap-2">
            ...
            EUI Causality Format Template Editor
          </h4>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
            ...
            <div className="bg-slate-900/40 border border-slate-800/80 rounded-xl p-5 space-y-4">
              ...
              <div className="space-y-2">
                {mainTitles.map((title) => {
                  const numLabel = getNumberDisplay(title.id);
                  return (
                    <div key={title.id} className="flex items-center gap-2 group">
                      {numLabel && (
                        <span className="text-[10px] font-mono text-slate-500 min-w-[20px] text-right font-bold select-none">
                          {numLabel}
                        </span>
                      )}
                      <input
                  ...
                  {driver.titles.map((title) => (
                    <div key={title.id} className="flex items-center gap-2 group">
                      <input
                        ...
                        className="flex-1 px-3 py-1.5 bg-slate-955 border border-slate-850 hover:border-slate-800 rounded-lg text-white placeholder-slate-650 focus:outline-none focus:ring-1 focus:ring-teal-500 focus:border-teal-500 text-xs transition-all font-semibold"
                      />
        ...
        {/* Live preview */}
        <div className={`space-y-3 transition-all duration-355 ${isEditMode ? "lg:col-span-5" : "lg:col-span-12"}`}>
              ...
              <div className="flex items-center gap-2 flex-wrap">
                ...
                {drivers.length > 0 && (
                  <button
                    ...
                    className="px-2.5 py-1 rounded border border-red-955/40 bg-transparent hover:bg-red-950/20 text-[10px] font-bold text-red-500 hover:text-red-400 transition-all cursor-pointer flex items-center gap-1 shadow-sm"
                  >
                    <Trash2 className="h-3 w-3" />
                    Remove Driver {String(drivers.length).padStart(2, "0")}
                  </button>
                )}
                {canManageTemplate && (
