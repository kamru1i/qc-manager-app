  ...
  return (
    <div className={`space-y-5 animate-pulse ${className}`}>
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
        ...
        <div className="h-9 w-36 bg-gradient-to-r from-blue-900/30 to-purple-900/30 border border-blue-800/20 rounded-xl" />
      </div>
      ...
      <div className="bg-slate-955/20 rounded-xl border border-slate-850 overflow-hidden">
        ...
        <div className="grid grid-cols-[2fr_1fr_1fr_1fr_1fr] gap-4 px-4 py-3 border-b border-slate-800 bg-slate-900/40">
          {['NAME / CODENAME', 'ROLE', 'LEAVE TRACKER', 'QUOTES TRACKER', 'FILE TYPE'].map((_, i) => (
            <div key={i} className={`h-2.5 ${bar} ${i === 0 ? 'w-3/5' : 'w-4/5 mx-auto'}`} />
          ))}
        </div>
        ...
        <div className="divide-y divide-slate-850">
          {Array.from({ length: rows }).map((_, i) => (
            <div key={i} className="grid grid-cols-[2fr_1fr_1fr_1fr_1fr] gap-4 px-4 py-3.5 items-center">
              {/* Name + Codename */}
              <div className="space-y-1.5">
                <div className={`h-3.5 ${bar}`} style={{ width: `${50 + (i % 5) * 10}%` }} />
